...
class MQTTClient:
    ...
    def wait_msg(self):
        ...
        if topic_len is None or len(topic_len) < 2:
            raise OSError(-1)
        topic_len = (topic_len[0] << 8) | topic_len[1]
        
        # Robustness check: Topics are usually short. 512 is plenty.
        if topic_len > 512:
            print("[umqtt] ERROR: Topic too long: {}".format(topic_len))
            raise OSError(-1)
        ...
        # Robustness check: Ensure sz is positive and not absurdly large
        if sz < 0:
            ...
            with open(temp_file, 'wb') as f:
                ...
                while remaining > 0:
                    ...
                    if not chunk or len(chunk) != chunk_size:
                        print("[umqtt] ERROR: Incomplete message chunk")
                        raise OSError(-1)
                    f.write(chunk)
                    remaining -= chunk_size
            
            self.cb(topic, temp_file)
            
        except Exception as e:
            print("[umqtt] Error streaming message to file: {}".format(e))
            raise OSError(-1)
        if op & 6 == 2:
